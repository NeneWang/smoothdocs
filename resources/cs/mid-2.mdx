...
<iframe
  width="100%"
  height="420"
  src="https://www.youtube.com/embed/tlPhSUdsHxw"
  allowfullscreen="true"
/>
...
<Tabs>
  <TabItem value="js+logs" label="Js+logs">
...
function getNextIdx(currentIdx, array){
  ...
  console.log(`currentIdx: ${currentIdx} arrel: ${array[currentIdx]} += ${nextIdx}`)
...
function hasSingleCycle(array) {
  ...
  while(amountOfTraversed < array.length){
    if (amountOfTraversed > 0 && currentIdx === 0) return false
...
function getNext(idx, array){
  nextIdx = (idx + array[idx])%array.length
  return nextIdx < 0? array.length + nextIdx: nextIdx
